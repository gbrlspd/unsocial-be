...
export class PostCache extends BaseCache {
  ...
  public async getUserPostsFromCache(key: string, uId: number): Promise<IPostDocument[]> {
    try {
      ...
      const replies: PostCacheMultiType = multi.exec() as unknown as PostCacheMultiType;
      const postReplies: IPostDocument[] = [];
      for (const post of replies as IPostDocument[]) {
        post.commentsCount = Helpers.parseJson(`${post.commentsCount}`) as number;
        post.reactions = Helpers.parseJson(`${post.reactions}`) as IReactions;
  ...
  public async getUserTotalPostsFromCache(uId: number): Promise<number> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }
  ...
  public async updatePostFromCache(key: string, updatedPost: IPostDocument): Promise<IPostDocument> {
